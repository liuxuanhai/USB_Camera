 ...
 * for record
 * MJPEG-->MJPEG(avi)
 * frame_buffer-->mjpeg_buffer
 */
uint8 *yuyv_buffer = NULL;
uint8 *mjpeg_buffer = NULL;
...
int video_preview_init(void)
{
	...
	ybuf = NULL;
	ybuf = (int *) calloc(1, sizeof(int) * camera->width * camera->height);
	ERROR(NULL == ybuf, err7, "calloc ybuf\n");

	mjpeg_buffer = NULL;
	mjpeg_buffer = (uint8 *)calloc(1, camera->width * camera->height * 2);
	ERROR(NULL == mjpeg_buffer, err8, "calloc mjpeg_buffer\n");
...
void video_get_preview_frame(void *pixels, int pixels_size)
{
	...
	//when we save MJPEG format data to AVI file, we just use camera->frame_buffer rather than yuyv_buffer
	pthread_mutex_lock(&record_buffer_mutex);
	ret = query_frame(camera);

	pthread_cond_signal(&record_buffer_cond);		//signal thread waiting for frame_buffer data
...
int video_record_uninit(void)
{
	...
	is_recording = FALSE;
	pthread_cond_signal(&record_buffer_cond);		//signal thread waiting for frame_buffer data to end waiting
...
//		if (camera->pixelformat == V4L2_PIX_FMT_MJPEG)
		{
			pthread_mutex_lock(&record_buffer_mutex);
			pthread_cond_wait(&record_buffer_cond, &record_buffer_mutex);		//wait for frame_buffer data ready

			memcpy(mjpeg_buffer, camera->frame_buffer, camera->frame_bytesused);
			bytes = camera->frame_bytesused;
